...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/05/21 06:57:52 by pgallois          #+#    #+#             */
/*   Updated: 2015/05/29 14:41:07 by pgallois         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	normal_sphere(t_ray *r, t_objet *p, float *t)
{
	...
	retvalue = 1;
	assign_value_vec(vector_sub(vector_add(*r->start,
					vector_dot_float(*t, *r->dir)), *p->ori), p->normali);
...
int normal_cylindre(float *tab, t_vec **vect, t_objet *p, float *t)
{
	...
	if (p->h != 0 && (((tab[3] == 0) && (fabs((vector_sub(*p->m_vec[5]\
		, *vect[2])).x) > p->h)) || ((tab[4] == 0) && (fabs((vector_sub(\
		*p->m_vec[5], *vect[2])).y) > p->h)) || ((tab[5] == 0)
		&& (fabs((vector_sub(*p->m_vec[5], *vect[2])).z) > p->h))))
		return (0);
