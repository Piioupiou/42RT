...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/05/21 05:54:24 by acrosnie          #+#    #+#             */
/*   Updated: 2015/05/21 05:54:24 by acrosnie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	lambert_function_color(t_data *d, int j, t_ray light_ray, t_vec *n)
{
	...
	if (temp != 0.0f)
	{
		assign_value_vec(vector_dot_float((1.0f / temp), *d->blinn_dir), d->blinn_dir);
...
t_vec	*calcul_light_shadow(t_data *d, float *t, t_vec *new_start, int c)
{
	int		j;
	t_vec	*n;
	...
	while (d->objet[++j])
	{
		...
		{
			assign_value_vec(vector_sub(*d->objet[j]->ori, *new_start), d->dist);
			if ((vector_dot(*n, *d->dist) <= 0.0f || ((*t = sqrtf(vector_dot(*d->dist, *d->dist))) <= 0.0f)))
				continue ;
			d->lray.start = new_start;
			d->lray.dir->x = 0;
			d->lray.dir->y = 0;
			d->lray.dir->z = 0;
			assign_value_vec(vector_dot_float((1 / *t), *d->dist), d->lray.dir);
			if (!d->option->activate_shadow || (col_object(d, t, &d->lray) == -1 && (d->current = c) == c))
				lambert_function_color(d, j, d->lray, n);
		}
		else if (!d->option->activate_light)
		{
			d->color->r = d->objet[d->current]->color->r;
			d->color->g = d->objet[d->current]->color->g;
			d->color->b = d->objet[d->current]->color->b;
		}
	}
...
void	calcul_next_iteration(t_data *d, t_ray *ray, t_vec *n, t_vec *new_start)
{
	...
	d->coef *= 0.5f;
	ray->start = new_start;
	assign_value_vec(vector_sub(*ray->dir, vector_dot_float(reflet, *n)), ray->dir);
	d->option->level++;
}
